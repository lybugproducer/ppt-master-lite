...
PPT Master is an AI-driven presentation generation system. Multi-role collaboration (Strategist → Executor) converts source documents (PDF/DOCX/URL/Markdown) into natively editable PPTX with real PowerPoint shapes (DrawingML).

**Core Pipeline**: `Source Document → Create Project → [Template] → Strategist Eight Confirmations → Executor → Quality Check → Post-processing → Export PPTX`

> AI image generation (`image_gen.py`), web image sourcing (`image_search.py`), web-page fetching (`web_to_md.py`), narration/video export (`notes_to_audio.py`, `--recorded-narration`), and repo updates (`update_repo.py`) are **permanently disabled** in this deployment — the pipeline is fully offline. All deck images are user-provided (`Acquire Via: user` / `placeholder`).
...
>
> Recorded narration / video export is **permanently disabled** in this deployment — `notes_to_audio.py` and `svg_to_pptx.py --recorded-narration` refuse to run (external TTS / video export off). Do not run the [`generate-audio`](skills/ppt-master/workflows/generate-audio.md) workflow.
>
...
# python3 skills/ppt-master/scripts/source_to_md/web_to_md.py <URL>   # permanently disabled (external fetch)
...
# image_gen.py / image_search.py are permanently disabled — all deck images are user-provided
