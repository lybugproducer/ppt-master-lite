...
In this deployment, AI image generation is permanently disabled. All deck images are user-provided: place your files in the project's `images/` folder and select the "User-provided" image approach during the Strategist phase. The pipeline scans the files and embeds them directly — no image generation tool is invoked.

## Q: I don't have an image-generation API key — can I still get images?

Yes — no API key is needed at all. AI image generation and web sourcing are permanently disabled in this deployment; every image comes from your own files. Put them in the project's `images/` folder and choose "User-provided" in the Strategist's Image Usage step — no API key, no external service.

## Q: Can PPT Master run fully offline (intranet, no external access)?

Yes. The core pipeline (local source files → project → SVG → PPTX) is fully local and never contacts external servers. In this deployment, every script that would reach the outside is disabled: web-page fetching (`web_to_md.py`), network image search, AI image generation, narration / video export (online TTS), and repo updates (`update_repo.py`). Install Python dependencies once through an internal PyPI mirror; after that the pipeline runs completely offline.
...
`on-click` is for live presentations. Narrated/video export via `--recorded-narration` rejects it because PPT Master writes page timings, not object-level click timings; use `after-previous` or `with-previous` for narrated decks. (Recorded narration / video export is permanently disabled in this deployment.)
