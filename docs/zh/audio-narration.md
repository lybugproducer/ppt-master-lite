...
> ⛔ **本部署已永久禁用**：旁白与视频就绪导出依赖外部 TTS 服务（`edge-tts` → 微软在线，或各家云端）。不要运行 `notes_to_audio.py`，不要使用 `svg_to_pptx.py --recorded-narration`，也不要触发 [`generate-audio`](../../skills/ppt-master/workflows/generate-audio.md) 工作流。仅保留通过 `--narration-audio-dir` 嵌入用户自备本地音频的能力。本文其余内容仅供存档参考。
...
| 命令 | 用途 |
|---|---|
| `--recorded-narration audio` | 准备 PowerPoint 的"录制的计时和旁白"。要求每页都有音频，并写入页面自动推进时间。用于旁白视频导出。—— **本部署已禁用** |
