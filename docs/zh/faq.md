...
本次部署中，AI 图片生成已被永久禁用，所有配图都来自你提供的素材。把图片放到项目的 `images/` 目录下，在策略师阶段选择"用户提供图片"即可——流程会扫描这些图片并直接嵌入页面，全程不调用任何生图工具。

## Q: 没有生图 API Key，还能配图吗？

可以，而且根本不需要 API Key。本次部署中 AI 生图和网络搜图都已永久禁用，所有配图都来自你自己的素材——把图片放进项目的 `images/` 目录，在策略师阶段选择"用户提供图片"即可。

## Q: 可以在完全离线（内网、无外网）环境运行吗？

可以。核心流程（本地源文件 → 项目 → SVG → PPTX）完全本地运行，不会访问任何外部服务器。本次部署中，所有需要外联的功能都已禁用：网页抓取（`web_to_md.py`）、网络搜图、AI 生图、旁白/视频导出（在线 TTS）和仓库更新（`update_repo.py`）。Python 依赖通过内网 PyPI 镜像安装一次后，生成流程即可完全离线运行。
...
`on-click` 适合现场演示。通过 `--recorded-narration` 做旁白/视频导出时会拒绝它，因为 PPT Master 只写页面级计时，不生成对象级点击计时；带旁白的 deck 请使用 `after-previous` 或 `with-previous`。（注：本部署中旁白/视频导出已禁用。）
...
- **用户提供的优质素材**：配图水平是 deck 整体观感的最大变量。使用贴合内容的实拍或设计素材，视觉效果远好于占位图。

如果你看到的效果差强人意，先对照以下几点检查你的配置，再下结论：用的什么模型？上下文开了多大？配图素材是否足够贴合内容？同样的工作流，Claude Opus 配 100 万 token 上下文的结果，和小参数开源模型配小上下文的结果，是截然不同的体验。
