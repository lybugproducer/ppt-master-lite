...
def _download_remote_image(url: str, media_dir: Path, index: int) -> str | None:
    """Remote image download — permanently disabled in this deployment.

    Never contacts an external server. Remote (http/https) images referenced
    by DOCX / HTML sources are skipped; only local and embedded images are kept.
    """
    print(
        f"[WARN] Skipping remote image (external fetch disabled): {url}",
        file=sys.stderr,
    )
    return None
