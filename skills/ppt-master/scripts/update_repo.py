...
PERMANENTLY DISABLED in this deployment: `git pull` and `pip install` access
external servers (GitHub / PyPI). `main()` refuses to run; the original
implementation is retained below as `_main_impl()` for reference.
...
def main() -> int:
    ...
    print(
        "update_repo.py is permanently disabled in this deployment. "
        "It pulls from the remote git origin and installs from PyPI, which "
