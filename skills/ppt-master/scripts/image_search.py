...
PERMANENTLY DISABLED in this deployment: web image sourcing is off and
`main()` refuses to run. All PPT images are user-provided (`Acquire Via: user`).
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    print(
        "image_search.py is permanently disabled in this deployment. "
        "Web image sourcing is off; PPT images must be user-provided "
        "(Acquire Via: user).",
        file=sys.stderr,
    )
    return 1
