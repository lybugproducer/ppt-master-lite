...
|------|-----------------|---------------|
| Conversion | `source_to_md/pdf_to_md.py`, `source_to_md/doc_to_md.py`, `source_to_md/excel_to_md.py`, `source_to_md/ppt_to_md.py` (`web_to_md.py` permanently disabled — external fetch) | [docs/conversion.md](./docs/conversion.md) |
...
| SVG pipeline | `finalize_svg.py`, `svg_to_pptx.py`, `total_md_split.py`, `svg_quality_checker.py`, `animation_config.py` (`notes_to_audio.py` / `--recorded-narration` permanently disabled) | [docs/svg-pipeline.md](./docs/svg-pipeline.md) |
| Spec maintenance | `update_spec.py` | [docs/update_spec.md](./docs/update_spec.md) |
| Image tools | `analyze_images.py`, `gemini_watermark_remover.py` (`image_gen.py` / `image_search.py` permanently disabled) | [docs/image.md](./docs/image.md) |
| Repo maintenance | `update_repo.py` (permanently disabled — git pull / pip install need external access) | README install/update section |
...
python3 scripts/source_to_md/excel_to_md.py <workbook.xlsx>
# web_to_md.py is permanently disabled (external fetch)
```
...
Image analysis (AI generation is permanently disabled — all images are user-provided):

```bash
python3 scripts/analyze_images.py <project_path>/images
```

Repository update: **disabled** — `update_repo.py` pulls from the remote git origin and PyPI, which require external access.
