...
> **Permanently disabled**: in this deployment, prompt-based AI generation (`image_gen.py`) and web image search (`image_search.py`) are disabled — the scripts refuse to run. All deck images are user-provided (`Acquire Via: user` / `placeholder`); only `analyze_images.py` remains active.
