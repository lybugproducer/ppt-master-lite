...
> **Permanently disabled in this deployment** — `notes_to_audio.py` (external TTS) and `--recorded-narration` (video export) refuse to run; local embedding via `--narration-audio-dir` remains available. Steps below are retained for reference only.
