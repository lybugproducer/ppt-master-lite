...
| -------- | --------- | ----- | ------- | ---- | ------ | --------------------- |
| cover_bg.png | {canvas_info['dimensions']} | [ratio] | Cover background | [Background/Photography/Illustration/Diagram/Decorative] | [Existing/Placeholder] | [User-provided image; `Acquire Via: user`] |

**Status**:

- **Existing** — user-supplied, place in `images/`
- **Placeholder** — not yet processed, use dashed border in SVG
- **Pending** — reserved for `Acquire Via: ai` / `web`; permanently disabled in this deployment, do not use

**Type** (used by Executor for layout handling):
