...
**Core Pipeline**: `Source Document → Create Project → [Template] → Strategist → Executor → Post-processing → Export`

> **Deployment policy**: AI image generation (`image_gen.py`) and web image sourcing (`image_search.py`) are **permanently disabled** in this deployment. All deck images are user-provided (`Acquire Via: user` / `placeholder`); Step 5 is always skipped.
...
| `${SKILL_DIR}/scripts/source_to_md/web_to_md.py` | Web page to Markdown — **permanently disabled** in this deployment (external fetch) |
| `${SKILL_DIR}/scripts/project_manager.py` | Project init / validate / manage |
| `${SKILL_DIR}/scripts/analyze_images.py` | Image analysis |
| `${SKILL_DIR}/scripts/image_gen.py` | AI image generation (multi-provider) — **permanently disabled** in this deployment |
...
| `visual-edit` | `workflows/visual-edit.md` | Browser-based visual editor — **permanently disabled** in this deployment |
...
| Web link / WeChat page | **Disabled in this deployment** (external web fetching is off). Save the page as HTML / Markdown and provide the local file instead. |
...
### Step 5: Image Acquisition Phase (Permanently Disabled)

🚧 **GATE**: Step 4 complete; Design Specification & Content Outline generated and user confirmed.

> **Deployment policy — permanently disabled**: AI image generation (`Acquire Via: ai`) and web image sourcing (`Acquire Via: web`) are permanently disabled in this deployment. Strategist MUST only write `Acquire Via: user` or `Acquire Via: placeholder` rows. This phase is therefore **always skipped** — do NOT load `references/image-base.md`, `references/image-generator.md`, or `references/image-searcher.md`, and do NOT run `image_gen.py` / `image_search.py` (both scripts refuse to run).

If a pre-existing spec or project still contains `ai` / `web` rows, reclassify each one as `user` (file supplied by the user) or `placeholder` (user will supply later). Never attempt acquisition.
...
🚧 **Image readiness GATE**: every image row must have its file at `project/images/<filename>` before running 7.1 (`ai` / `web` rows never occur — all images are user-provided; `placeholder` rows are intentional gaps the user fills later).

> If a user-provided image is missing: PAUSE, list the missing filenames, and ask the user to place the files at `project/images/<filename>`. Resume Step 7.1 only after all expected files are in place. `finalize_svg.py` and `svg_to_pptx.py` do not detect missing files at this layer — proceeding with gaps produces a deck with broken image references.
...
**Recorded narration / video export — permanently disabled**:

Narration and video-ready export are disabled in this deployment. `notes_to_audio.py` relies on external TTS services (`edge-tts` → Microsoft online, or cloud providers) and refuses to run; `svg_to_pptx.py --recorded-narration` is rejected as well. Do NOT run the [`generate-audio`](workflows/generate-audio.md) workflow. If a deck needs narration, provide local audio files and embed them with `--narration-audio-dir` (no external service involved).
