...
**Recorded narration / video export — permanently disabled**: `notes_to_audio.py` (external TTS) and `svg_to_pptx.py --recorded-narration` (video-ready export) refuse to run in this deployment. Local narration embedding via `--narration-audio-dir` remains available.
