...
| **B** | User-provided | Has existing image assets — **mandatory when images are used** |
| **E** | Placeholders | Images the user will supply later |

> **Hard rule — permanent deployment policy**: AI image generation (`Acquire Via: ai`) and web image sourcing (`Acquire Via: web`) are **permanently disabled**. Strategist MUST NOT propose options C / D, MUST NOT create `ai` / `web` rows, and MUST NOT mention `image_gen.py` / `image_search.py`. Every image in the deck is user-provided (`user`) or a placeholder the user fills later (`placeholder`).

> Note: no AI-generation mode table applies — options C and D are disabled by policy (see the hard rule above).

**When the deck uses images**, you must run `python3 scripts/analyze_images.py <project_path>/images` before outputting the spec, and integrate scan results into the image resource list.

**When B / E is selected**, add an image resource list to the spec:
...
| **Acquire Via** | `user` / `placeholder` only — `ai` / `web` are permanently disabled and MUST NOT appear |
| Status | Initial status must be `Existing` (file already in `images/`) or `Placeholder` (user will supply later); see [`svg-image-embedding.md`](svg-image-embedding.md) for the full status enum |
| **Reference** | Free-form **description** of what the image shows and where it is used |
...
**Per-row Reference pattern** (all rows are `user` / `placeholder`):

| Acquire Via | Reference pattern |
|---|---|
| `user` / `placeholder` | What the image shows and where it is used on the slide — factual, not a generation prompt |
...
> **Pipeline handoff**: All rows are `user` / `placeholder` — Step 5 (Image Acquisition) is permanently skipped and Executor proceeds directly with the files in `project/images/`. Status names are defined in [`svg-image-embedding.md`](svg-image-embedding.md).
...
Next step:
- All images are user-provided → Invoke Executor
```
...
Next step:
- All images are user-provided → Invoke Executor (free design for every page)
```
