...
|----------|------------|---------|------|-------------|--------|-----------|
| cover_bg.png | 1280x720 | Cover background | Background | user | Existing | User-supplied cover image |
| product.png | 600x400 | Page 3 product photo | Photography | user | Existing | - |
| chart.png | 600x400 | Page 5 placeholder | Illustration | placeholder | Placeholder | Image the user will supply later |
```
...
|--------|---------|-------------------|
| **Pending** | Acquisition needed (`Acquire Via: ai` / `web`) — **disabled in this deployment**; do not create | — |
| **Generated** | AI-generated file exists — **disabled in this deployment** | — |
| **Sourced** | Web-sourced file exists — **disabled in this deployment** | — |
| **Needs-Manual** | Acquisition attempted and failed — **disabled in this deployment** | Dashed placeholder unless the user supplies the file |
...
2. Image Acquisition (Step 5) — **permanently disabled** in this deployment. `ai` / `web` rows are forbidden; only `user` / `placeholder` rows exist and Executor uses them directly.
3. Executor generates SVGs (svg_output/)
   ├── Existing → <image href="../images/xxx.png" .../>
   └── Placeholder without file → Dashed border + description text
