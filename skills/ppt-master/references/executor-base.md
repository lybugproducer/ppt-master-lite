...
| **Generated** | Generated by Image_Generator — **disabled in this deployment** | — |
| **Sourced** | Web-acquired by Image_Searcher — **disabled in this deployment** | — |
| **Needs-Manual** | Acquisition failed and file is absent — **disabled in this deployment** | Use dashed border placeholder unless the user supplies the file |
...
**Pure spoken narration**: notes are read aloud verbatim by `notes_to_audio.py` (TTS — permanently disabled in this deployment). Write only what should be spoken. No visible markers, no labeled meta-lines, no enumerated key-point lists, no duration annotations — anything you write outside the heading will be vocalized.
