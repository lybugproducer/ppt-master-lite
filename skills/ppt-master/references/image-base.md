...
> **Note — permanently disabled**: `Acquire Via: ai` and `Acquire Via: web` are forbidden in this deployment. Strategist writes only `user` / `placeholder` rows, so this phase never activates. The file is retained for status / format reference only.

---

## 1. Trigger Condition

**Inactive by policy.** Resource list rows may only carry `Acquire Via: user` or `Acquire Via: placeholder`; `ai` / `web` rows MUST NOT exist. There is no dispatch to run.
...
**Disabled — `ai` and `web` rows never exist.** Active rows are:
...
- Every non-skipped row has a file at `project/images/<filename>`, or is a `Placeholder` the user will supply later
- No `ai` / `web` rows exist (permanently disabled)
